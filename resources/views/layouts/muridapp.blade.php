...
<head>
  ...
  <title>@yield('title', 'Murid Dashboard')</title>
...
<body class="bg-gray-100">
  ...
  <nav class="bg-gradient-to-r from-gray-800 to-gray-900 p-4 md:hidden flex justify-between items-center shadow-md">
    ...
    <div class="text-white text-xl font-semibold">Murid Dashboard</div>
  </nav>
  ...
  <aside id="sidebar" class="sidebar bg-gray-900 text-white w-72 space-y-8 py-8 px-4 fixed inset-y-0 left-0 transform -translate-x-full md:translate-x-0 shadow-2xl z-50">
    <div class="text-center">
      <h2 class="text-3xl font-bold mb-2">Murid Dashboard</h2>
      <p class="text-sm text-gray-300">{{ auth()->user()->name ?? 'Guest' }}</p>
    </div>
    <nav class="space-y-4">
        <a href="{{ route('murid.dashboard') }}" class="flex items-center px-6 py-3 rounded-lg hover:bg-gray-700 transition {{ request()->routeIs('murid.dashboard') ? 'bg-blue-600' : '' }}">
            <i class="fas fa-tachometer-alt text-xl"></i>
            <span class="ml-4">Dashboard</span>
        </a>
        <a href="{{ route('murid.jadwal.index') }}" class="flex items-center px-6 py-3 rounded-lg hover:bg-gray-700 transition {{ request()->routeIs('murid.jadwal.index') ? 'bg-blue-600' : '' }}">
            <i class="fas fa-calendar-alt text-xl"></i>
            <span class="ml-4">Jadwal</span>
        </a>
        <a href="{{ route('murid.materi.index') }}" class="flex items-center px-6 py-3 rounded-lg hover:bg-gray-700 transition">
