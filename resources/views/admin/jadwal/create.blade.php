  ...
  <form action="{{ route('jadwal.store') }}" method="POST" class="space-y-6">
    ...
    <div>
      ...
      <select name="guru_id" id="guru_id" class="w-full border border-gray-300 px-4 py-2 rounded-lg focus:outline-none focus:border-blue-500 @error('guru_id') border-red-500 @enderror">
        ...
        @foreach($guru as $guruItem)
          <option value="{{ $guruItem->id }}" {{ old('guru_id') == $guruItem->id ? 'selected' : '' }}>
            {{ $guruItem->name }}
          </option>
        @endforeach
      </select>
    ...
    <div>
      ...
      <select name="pelajaran_id" id="pelajaran_id" class="w-full border border-gray-300 px-4 py-2 rounded-lg focus:outline-none focus:border-blue-500 @error('pelajaran_id') border-red-500 @enderror">
        ...
        @foreach($pelajaran as $pelajaranItem)
          <option value="{{ $pelajaranItem->id }}" {{ old('pelajaran_id') == $pelajaranItem->id ? 'selected' : '' }}>
            {{ $pelajaranItem->nama }}
          </option>
        @endforeach
      </select>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        ...
        <input type="time" name="jam_mulai" id="jam_mulai" value="{{ old('jam_mulai') }}" class="w-full border border-gray-300 px-4 py-2 rounded-lg focus:outline-none focus:border-blue-500 @error('jam_mulai') border-red-500 @enderror">
        @error('jam_mulai')
          <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        @enderror
      </div>
      <div>
        ...
        <input type="time" name="jam_selesai" id="jam_selesai" value="{{ old('jam_selesai') }}" class="w-full border border-gray-300 px-4 py-2 rounded-lg focus:outline-none focus:border-blue-500 @error('jam_selesai') border-red-500 @enderror">
        @error('jam_selesai')
          <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        @enderror
      </div>
