        ...
        <tbody>
          @forelse($jadwal as $item)
            <tr class="hover:bg-gray-50">
              ...
              <td class="px-4 py-2 border-t">{{ $item->kelas->name ?? 'N/A' }}</td>
              <td class="px-4 py-2 border-t flex space-x-2">
                <a href="{{ route('jadwal.show', $item->id) }}" class="text-blue-500 hover:text-blue-600">Lihat</a>
          ...
          @empty
            <tr>
              <td colspan="3" class="text-center px-4 py-2 border-t">Tidak ada data jadwal.</td>
            </tr>
          @endforelse
        </tbody>
      </table>
    ...
    <div class="flex items-center justify-between border-t border-gray-200 bg-white px-4 py-3 mt-4">
      <div class="flex flex-1 justify-between sm:hidden">
        @if($jadwal->previousPageUrl())
          <a href="{{ $jadwal->previousPageUrl() }}" class="relative inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Previous</a>
        @endif
        @if($jadwal->nextPageUrl())
          <a href="{{ $jadwal->nextPageUrl() }}" class="relative ml-3 inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Next</a>
        @endif
      </div>
